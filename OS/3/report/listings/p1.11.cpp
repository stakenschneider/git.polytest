...
int main() {
	...
	if(sched_rr_get_interval(0, &time) == 0) {
		std::cout << "Parent " << double(time.tv_nsec) / SECONDS_COEFF + time.tv_sec << " seconds." << std::endl;
	}
	else {
	...
	if(!fork()) {
		...
		// Определение длины кванта
		if(sched_rr_get_interval(0, &time) == 0) {
			std::cout << "Child " << double(time.tv_nsec) / SECONDS_COEFF + time.tv_sec << " seconds." << std::endl;
