...
static constexpr char* READER_PROCESS_PATH = "p2.3.r.exe";
...
int readersDontReadyToRead, readersAlreadyRead;
// Отображение файла
HANDLE fileMapping;
...
int main(int argc, char* argv[]) {
	...
	*(((int*) writersMap) + 1) = globalConfig.readersCount;
	...
	eventCount = CreateEvent(nullptr, false, false, EVENT_COUNT_NAME);
...
void createAllThreads(Configuration * config) {
	...
	for(int readerIndex = 0; readerIndex < config->readersCount; ++readerIndex) {
		...
		BOOL reader = CreateProcess(nullptr, _strdup(readerPath.data()), nullptr, nullptr, FALSE, CREATE_NEW_CONSOLE | CREATE_SUSPENDED, nullptr, nullptr, &startupInformation, &processInformation);
		if(!reader) {
			std::cerr << "It's impossible to create reader." << std::endl;
			exit(0x5);
		}

		handlers.push_back(processInformation.hThread);
	}
...
DWORD WINAPI threadWriterExecutor(LPVOID argument) {
	...
	while(!interrupt) {
		readersDontReadyToRead = globalConfig.readersCount;
		readersAlreadyRead = globalConfig.readersCount;

		while(true) {
			WaitForSingleObject(eventCount, INFINITE);

			--readersDontReadyToRead;
			if(readersDontReadyToRead == 0) {
				ResetEvent(eventAllRead);
				SetEvent(eventCanWrite);
				ResetEvent(eventCount);
		...
		while(true) {
			...
			--readersAlreadyRead;
			if(readersAlreadyRead == 0) {
				// Если последнее чтение, то запрещаем читать
