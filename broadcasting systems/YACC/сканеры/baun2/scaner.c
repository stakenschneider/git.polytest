	...
	for (state = 0; ; ) {
		...
		switch(state) {
		   case 0: 				/* skip whitespaces */
			...
			if (isdigit(c)) {	
				state = 3;		/* treat next chars as Number */
				break;
		...
		case 3:
		  if (isxdigit(c)){
		    ...
		    if (c=='h'){
		      ...
		      /* not ok - need unput all */
		      //fprintf(stderr, "need unput %d chars\nunputed: ", count);
		      /* unput here */
		      unput(c);
		      int i;
		      for (i=count;i>0;i--){
			unput (buf[i-1]);
			//fprintf(stderr, "%d:%s ", i-1, buf[i-1]);
		      }
		      count=i;
		      //fprintf(stderr, "unput end. counter=%d\n", count);
		      state=2;
		      break;
		    }
		  }
