klavIn:
 ...
 ; SLAVE не имеет права ничего нажимать
	...
	clr C
	subb a, #53h	; S-Slave
	jz s_end	; А!=0 => следующая проверка
 ...
 ; ****** button15 #0fh=15 (установка режима "Master/Slave")
	...
	clr C
	subb a, #0Fh	
	jnz s_m1; А!=0 => следующая проверка
 ...
 ; Обычный режим - никак особых действий
s_m1:	mov a, status
	clr C
 ...
 ; ****** button14 #0Eh=14 (передача адреса)
	...
	clr C
	subb a, #0Eh	
	jnz s_m2	; А!=0 => следующая проверка

	; если нажата 14
	setb TB8	; установили бит=>передаем адрес

	mov a, #31h	; #31h="1"
	mov dptr, #str2+6	; на место 1го Х
...
s_m2:	mov a, button
	clr C
	subb a, #0Dh	
	jnz s_m3	; А!=0 => следующая проверка
...
s_m3:	mov a, button
	clr C
	subb a, #0Ch	
	jnz s_next	; А!=0 => никаких действий,
			; просто передача
...
klavIn2:
 ; MASTER ничего не принимает
	...
	clr C
	subb a, #4Dh	; M-Master
	jz r_end	; А!=0 => следующая проверка
 ...
 ; ****** button15 #0fh=15 (установка режима "Master/Slave")
	...
	clr C
	subb a, #0Fh	
	jnz r_m1	; А!=0 => следующая проверка

	; если была нажата 15
	setb SM2	; особый режим прослушивания
...
r_m1:	mov a, status
  ...
  ; ****** button13 #0Dh=13 (передача адреса)
	...
	clr C
	subb a, #0Dh	
	jnz r_m2	; А!=0 => следующая проверка
...
r_m3:	mov a, button2
	clr C
	subb a, #0Ch	
	jnz r_next	; А!=0 => никаких действий,
			; просто передача
